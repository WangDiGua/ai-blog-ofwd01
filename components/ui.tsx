...
// --- Card Component ---
// Implements the Apple-style clean white card with soft shadow
export const Card = ({ children, className = '', hover = false }: { children: React.ReactNode; className?: string, hover?: boolean }) => {
  return (
    <div className={`
      bg-apple-card rounded-2xl border border-gray-100 shadow-sm overflow-hidden
...
export const Button = ({ children, variant = 'primary', size = 'md', className = '', ...props }: ButtonProps) => {
  ...
  const variants = {
    ...
    ghost: "bg-transparent text-apple-blue hover:bg-blue-50/50"
  };
...
export const Avatar = ({ src, alt, size = 'md' }: { src: string; alt: string; size?: 'sm' | 'md' | 'lg' }) => {
  ...
  return (
    <img 
      src={src} 
      alt={alt} 
      className={`${sizes[size]} rounded-full object-cover border border-gray-100 shadow-sm`}
    />
  );
};
