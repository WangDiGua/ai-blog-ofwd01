...
export const Timeline = () => {
    ...
    return (
        <div className="max-w-5xl mx-auto px-4 py-10 mb-20 overflow-hidden">
            <div className="text-center mb-24 animate-in fade-in slide-in-from-bottom-4 duration-700 relative z-10">
                ...
                {/* 装饰性背景文字 */}
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[120px] md:text-[200px] font-bold text-gray-100 dark:text-gray-800/20 -z-10 select-none whitespace-nowrap opacity-50">
                    TIMELINE
                </div>
            </div>
            ...
            <div className="relative">
                {/* 中间线 (桌面端) / 左侧线 (移动端) */}
                <div className="absolute left-4 md:left-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-transparent via-gray-300 dark:via-gray-700 to-transparent -translate-x-1/2 md:translate-x-0 z-0"></div>
                ...
                <div className="space-y-24 md:space-y-32">
                    {articles.map((article, index) => {
                        ...
                        return (
                            ...
                            >
                                {/* 时间点标记 (Parallax Float 让它与卡片移动速度错开) */}
                                ...
                                <div className={`w-full md:w-5/12 pl-12 md:pl-0`}>
                                    ...
                                    >
                                        ...
                                        >
                                            {/* 连接线 */}
                                            <div className={`absolute top-10 border-t border-dashed border-gray-300 dark:border-gray-700 w-8 md:w-16 opacity-50
                                                ${isEven ? 'left-[-32px] md:left-auto md:right-[-64px]' : 'left-[-32px] md:left-[-64px]'}
                                            `}></div>

                                            {/* 日期浮标 */}
                                            <div className={`absolute -top-8 ${isEven ? 'md:right-0' : 'md:left-0'} left-0 text-sm font-mono text-gray-400 flex items-center`}>
                                                <Clock size={12} className="mr-1" /> {article.date}
                                            </div>

                                            <Card className="overflow-hidden hover:shadow-2xl transition-shadow duration-500 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 p-0 rounded-2xl group/card">
                                                {/* 视差图片容器 */}
                                                ...
                                                <div className="p-6">
                                                    <h3 className="text-xl font-bold text-apple-text dark:text-apple-dark-text mb-3 leading-tight group-hover:text-apple-blue transition-colors">
                                                        {article.title}
                                                    </h3>
